...
③ 방법별 평가 루프 (BM25 → Embedding → Hybrid)
   각 쿼리에 대해:
     a. searcher.search(query_text, method=method, top_k=5, odd_filter=odd_hint) 호출
        ※ Hybrid의 경우 extract_odd_hint(query)로 ODD 힌트 추출 후 전달 (Phase A 픽스)
...
> **주의**: 이 섹션의 Hybrid 수치는 Gap-2 버그 버전(Hybrid=Embedding)이다. 픽스 후 수치(Hybrid=0.6600)는 섹션 11 참조.

### Recall@5 / MRR@5 요약 테이블 (Gap-2 버그 버전)

| 방법 | Recall@5 | MRR@5 | 지연 (중앙값) | 지연 (평균) |
|------|----------|-------|------------|----------|
| **BM25** | **0.8533** | **0.9361** | 0.83ms | 1.05ms |
| Embedding | 0.8267 | 0.9208 | 5.28ms | 5.32ms |
| Hybrid | ~~0.8267~~ (=Embedding) | ~~0.9208~~ | 5.13ms | — |

### 레벨별 Recall@5 (Gap-2 버그 버전, Hybrid=Embedding)
...
### 레벨별 MRR@5 (BM25 vs Embedding 기준 — Hybrid 제외)

| 레벨 | BM25 | Embedding | Emb-BM25 gap |
|------|------|-----------|-------------|
| L0 | 1.0000 | 0.9500 | **-0.0500** |
| L1 | 1.0000 | 1.0000 | 0.0000 |
| L2 | 0.8222 | **0.9111** | **+0.0889** |
| L3 | 0.9500 | 0.9500 | 0.0000 |
| L4 | **0.8667** | 0.6167 | **-0.2500** |
...
| **Hybrid** | Gap-2 버그 확인 → Phase A에서 픽스 완료 (섹션 11) |

### 검증 기준 (Phase A 이전, 초기 평가 기준)

| 기준 | 목표 | 결과 |
|------|------|------|
| 평가셋 크기 | 쿼리 ≥ 60개, 쿼리당 정답 ≥ 5개 | ✅ 60개, 평균 78.1개/쿼리 |
| Axis A: Embedding vs BM25 (L0) | Embedding Recall@5 ≥ BM25 | ❌ 0.880 < 0.930 |
| Axis A: 인과 쿼리 격차 | L2 gap > L0 gap | ✅ 0.000 > −0.050 (PASS) |
| Gap-2 수정 | Hybrid ≠ Embedding | ✅ Phase A에서 완료 (섹션 11) |
| 지연 측정 | 워밍업 제외 중앙값 200ms 이하 | ✅ BM25 0.83ms, Embedding 5.28ms |

> Phase A 이후 갱신된 검증 기준과 잔여 액션은 **섹션 14** 참조.
...
### 검증 기준 최종 현황 (Phase A 완료 기준)

| 기준 | 목표 | 결과 |
|------|------|------|
| 평가셋 크기 | 쿼리 ≥ 60개, 쿼리당 정답 ≥ 5개 | ✅ 60개, 평균 78.1개/쿼리 |
| Axis A: 인과 쿼리 격차 | L2 gap > L0 gap | ✅ PASS (Recall +0.000 > −0.050, MRR +0.089 > −0.050) |
...
| Axis A: Embedding vs BM25 (L0) | Embedding Recall@5 ≥ BM25 | ❌ 0.880 < 0.930 — L0에서는 BM25 우위 유지 |

### 잔여 액션 (Phase B 이전)

1. **L4 Embedding 약점**: L4-01·L4-02 실패 클립 캡션 확인 — bge-m3 임베딩에서 두 표현의 코사인 거리 측정
2. **L2 BM25 약점**: L2-07·L2-09 실패 클립 캡션 확인 → 캡션 어휘 표준화 가능성 검토
3. **노이즈 22% 분석**: 65,893개 노이즈 포인트 캡션 샘플링 → 극단 희귀 씬 식별
4. **top-k 확장**: `--top-k 10` 재평가로 Recall@10 곡선 확인
