...
## 3. SANFlow latent 공간을 사용한 이유

"UMAP 공간에서 바로 밀도를 계산하면 되는데 왜 굳이 flow로 변환하는가?"  
이 선택에는 명확한 이유가 있으며, 버그와 해결 방향을 이해하는 데도 중요하다.
...
NF는 학습 후 한 번의 forward pass로 **정확한** log p(x)를 계산한다.  
KDE는 본질적으로 근사이며 고차원에서 bandwidth 선택이 어렵다.
...
클러스터 크기에 관계없이 **클러스터 내에서의 이상도**를 측정하므로 크기 편향이 없다.

### 3-3. 이유 ③: 비선형 밀도 구조 포착

UMAP 공간의 클러스터들은 구형(spherical)이 아니다:
...
KDE는 각 포인트 주변에 동일한 구형 커널을 씌우므로  
비구형·비균질 분포에서 밀도를 과소/과대 추정한다.

Flow의 비선형 변환은 이런 복잡한 형태의 클러스터도  
latent 공간에서 단순한 Gaussian으로 "펼쳐" 더 정확한 밀도를 추정할 수 있다.

### 3-4. 이유 ④: 시나리오 역추적 (가장 핵심적인 동기)

이것이 SANFlow를 **표준 Normalizing Flow**와 구분짓는 핵심이다.
...
SANFlow                 ✅ 가능      ✅ 가능 (클러스터별 Gaussian → nearest μ_k → 시나리오 이름)
```

```
[표준 NF]                            [SANFlow]
  모든 데이터 → flow → z ~ N(0, I)   cluster k의 데이터 → flow → z ~ N(μ_k, σ_k)
  
  갭 = z가 N(0,I)에서 멀리 있는 점   갭 = z가 어느 N(μ_k, σ_k)에서도 멀리 있는 점
  → "이 클립이 희귀하다"는 알지만    → "이 클립은 cluster k 시나리오의 희귀 케이스"
    어떤 시나리오인지 알 수 없음       라는 역추적까지 가능
```

### 3-5. 정리: 두 공간의 역할 분담

이번 버그를 통해 드러난 더 중요한 통찰은 다음이다:
...
역추적은 **UMAP 원본 공간**에서 수행하는 것이 맞다.  
현재 버그는 역추적까지 latent 공간에서 한 것에서 비롯됐다.

---

## 4. 정량적 규모 (버그 영향 범위)

### 4-1. UMAP 공간 vs SANFlow latent 공간 nearest cluster 비교
...
### 4-3. noise 갭의 실제 UMAP nearest cluster 분포 (상위)
