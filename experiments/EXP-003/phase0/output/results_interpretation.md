...
**실행일**: 2026-07-03 | **데이터**: 54,912 clips | **출력 경로**: `experiments/EXP-003/phase0/output/`

---

## 1. 핵심 진단 — 데이터셋 성격

> **결론: 54,912개 클립이지만 실질적으로는 3,458개 분량의 정보만 담긴 극도로 중복적인 데이터셋**

| 지표 | 값 | 의미 |
|------|-----|------|
| Effective N (soft) | **3,458** (6.3%) | 실질 독립 클립 수 — 나머지 93.7%는 이미 있는 정보의 반복 |
| Vendi random | **3.513** ± 0.011 | 현재 분포 그대로 — 3~4개 의미 차원만 커버 |
| Vendi dedup | **3.638** ± 0.016 | 중복 제거 후 분포 — 억압 계수 1.036 (3.6% 향상에 그침) |
| Vendi topk | **4.742** ± 0.010 | 상위 Effective N 풀 상한 — 이상적 dedup 후 최대 기대치 |
| 상위 10이웃 유사도 중앙값 | **0.9419** | 모든 클립이 서로 94% 수준으로 유사 |
| Q1 PRUNE | **60.1%** | 과반이 "밀집+단조" — 중복 제거 대상 |
| healthy_scenarios | **0개** | 12개 시나리오 중 Q0 지배 시나리오 없음 |
| ODD 고유 조합 (전체) | **422 / 25,920 (1.6%)** | 7개 ODD 차원 조합 커버리지 극도로 좁음 |
| ODD effective N (전체) | **37.4** | 422개 조합 중 실질 37개 수준만 고르게 분포 |
| ODD effective N (topk) | **41.2** | 고유성 상위 3,458개 기준 실질 ODD 조합 수 |
| ODD mean_norm_entropy (전체) | **0.413** | 전체 클립 기준 ODD 다양성 |
| ODD mean_norm_entropy (topk) | **0.437** | 고유성 상위 클립이 ODD 관점에서도 더 다양 |
| ODD 임베딩 정렬 (쌍별 Spearman) | **ρ=0.237 (partial)** | 임베딩 거리 ↔ ODD 거리 약한 정렬 |
| Vendi-ODD run 정렬 (n=30) | **ρ≈0 (3전략 공통)** | 동일 앵커 기준 Vendi ↔ ODD entropy 무상관 |
...
|------|-----|
| N 전체 클립 수 | 54,912 |
| 상위 20이웃 평균 유사도 (전체 평균) | ≈ 0.9371 |
| 클립당 평균 고유성 가중치 | `1 - 0.9371` ≈ **0.063** |
| Effective N | `54,912 × 0.063` ≈ **3,458** ✓ |
...
|------|------|------|------------|
| **Soft** | $w_i = 1 - \bar{s}_i$ | 연속적 유사도 평균 | **3,458** |
| **Hard** | $w_i = \frac{1}{1 + \text{count}_{>0.95}}$ | sim > 0.95인 이웃 개수 | 34,029 |
...
#### vendi_random = 3.513 ± 0.011

```
전체 54,912개에서 균등 무작위 2,000개 선택
모든 클립 선택 확률 동일 = 1/54,912
```

중복 클립이 54,912개 중 대다수이므로, 무작위 2,000개 안에도 중복 클립이 비례적으로 포함됩니다. 유사한 벡터가 반복될수록 커널 행렬 랭크가 낮아지고 Vendi가 낮아집니다. **현재 분포 그대로, 모델이 실제 학습 시 받는 다양성 신호를 측정**합니다.

#### vendi_dedup = 3.638 ± 0.016

```
전체 54,912개에서 uniqueness_weight(= 1 - soft_commonness) 비례 샘플링
흔한 클립(이웃과 유사도 높음) → 선택 확률 낮음
희귀 클립(이웃과 유사도 낮음) → 선택 확률 높음
```

중복이 많은 클립일수록 가중치가 낮아져 앵커에 덜 포함됩니다. **SoftDedup을 완전히 적용한 이상적 데이터셋의 다양성을 시뮬레이션**합니다. std(0.016)가 random보다 약간 높은 이유: 희귀 클립은 수가 적으므로 선택 여부에 따라 Vendi가 random보다 조금 더 흔들립니다.

#### vendi_topk = 4.742 ± 0.010

```
1단계: uniqueness_weight 상위 3,458개(= Effective N) 클립을 고정 풀로 추출
2단계: 그 풀 안에서 균등 무작위 2,000개 선택
```

Effective N = 3,458은 "이 데이터셋에서 실질적으로 독립적인 클립 수 추정치"이므로, 상위 3,458개는 데이터셋의 모든 다양성이 응축된 집합입니다. 이 풀만으로 학습할 때의 **다양성 상한**을 측정합니다. 풀(3,458) > 앵커(2,000)이므로 여전히 무작위성이 있지만 풀 자체가 이미 고유 클립으로 구성되어 std(0.010)가 가장 낮습니다.
...
run 1: 무작위 2,000개 → Vendi = 3.5027
run 2: 다른 2,000개   → Vendi = 3.5013
run 3:                → Vendi = 3.5207
run 4:                → Vendi = 3.5171
run 5:                → Vendi = 3.5245
              ↓
SE/mean = std/√5 / mean = 0.011 / 3.513 = 0.31% < 2% 수렴 기준 충족 → 중단
              ↓
결과: mean=3.513, std=0.011  ← 이것이 3.513 ± 0.011
```
...
|------|------|-------|--------------------------|
| vendi_random | 3.513 | ± 0.011 | 54,912 균등 샘플 → 안정적 |
| vendi_dedup  | 3.638 | ± 0.016 | 희귀 클립 과대 표집 → run마다 편차 조금 큼 |
| vendi_topk   | 4.742 | ± 0.010 | 풀 자체가 고유 클립만 → 가장 안정적 |
...
| 비교 | 격차 | 의미 |
|------|------|------|
| dedup − random | **+0.125 (3.6%)** | 중복 제거로 얻을 수 있는 다양성 이득 |
| topk − random | **+1.229 (35%)** | 커버리지 확장으로 얻을 수 있는 다양성 이득 |
| topk − random / dedup − random | **10배** | 커버리지 문제가 중복 문제보다 10배 더 큰 원인 |

**억압 계수(dedup/random) = 1.036**: 중복을 완벽히 제거해도 다양성은 3.6% 향상에 그칩니다.

> **진단**: 다양성 부족의 원인은 "중복"이 아닌 **"수집하지 못한 시나리오"** — 중복 제거보다 새 시나리오 수집·합성이 10배 더 효과적입니다.

### Vendi = 3.513의 의미

54,912개 클립이 있지만, 임베딩 공간에서 이 데이터는 사실상 **3~4개의 독립적인 의미 방향**만 커버합니다.
...
시나리오별 Vendi 평균 2.7 (CV=0.075)도 같은 맥락 — **각 시나리오 내부에서도 2~3개 방향만 존재하며, 시나리오 간 편차도 거의 없음**.
...
| 지표 | 무엇을 측정 | 이번 값 |
|------|-----------|---------|
| **Effective N** | 개별 클립의 고유성 가중치 합 — *얼마나 많은 독립 클립이 있나* | 3,458 (6.3%) |
| **Vendi Score** | 고유값 스펙트럼 엔트로피 — *몇 개의 독립 방향을 커버하나* | 3.513 (random) |
...
| 항목 | 값 | 해석 |
|------|-----|------|
| Spearman ρ | **0.2369** | partial — 임베딩이 ODD 변화를 부분적으로 포착 |
...
**결론**: ρ=0.24는 임베딩이 ODD 조건 차이를 **약하게만** 포착함을 의미합니다. 따라서 Vendi=3.5는 ODD 다양성과 캡션 의미 다양성의 혼합 신호이며, "ODD가 3~4개 방향만 커버된다"는 결론의 **완전한 근거로 사용하기에는 유보가 필요**합니다.
...
|---------|---------|-------------------|--------------------|-------------------|------|
| lighting | 3 | **0.909** | 2.71 | 0.821 | ↓ |
| road_type | 6 | 0.591 | 2.88 | 0.654 | ↑ |
| road_divider | 5 | 0.581 | 2.55 | 0.547 | ↓ |
| traffic_density | 4 | 0.487 | 1.97 | 0.504 | ↑ |
| weather | 4 | 0.206 | 1.33 | 0.299 | ↑ |
| agent_type | 6 | **0.083** | 1.16 | 0.168 | ↑ |
| scene_ambiguity | 3 | **0.035** | 1.04 | 0.067 | ↑ |
| **평균** | | **0.413** | | **0.437** | ↑ |
...
| **agent_type 0.083 / eff=1.16** | ⚠️⚠️ 6종 있지만 사실상 cars_only 단일 카테고리. 버스·트럭·자전거·보행자 극소수 |
| **scene_ambiguity 0.035 / eff=1.04** | ⚠️⚠️ 3등급 있지만 전부 low(모호성 없음)에 몰림. 사실상 단일값 |

- **전체 54,912개**: 고유 ODD 조합 422개 (25,920 가능 조합의 1.6%), **odd_effective_n = 37.4**
- **고유성 상위 3,458개 (topk)**: 고유 ODD 조합 219개, **odd_effective_n = 41.2**
...
topk 219개의 effective_n=41.2가 전체보다 높은 이유: 조합 수는 줄었지만 남은 219개 조합의 **분포가 더 균등**하기 때문입니다 (많이 쏠린 조합들이 topk에서 걸러짐).
...
| 전략 | 앵커 모집단 | Vendi 범위 | ODD H 범위 | Spearman ρ | p값 |
|------|-----------|-----------|-----------|-----------|-----|
| random | 전체 54,912개 | 3.484~3.547 | 0.421~0.470 | **-0.04** | 0.83 |
| dedup | 전체 54,912개 (고유성 비례) | 3.594~3.663 | 0.429~0.486 | **0.01** | 0.95 |
| topk | 고유성 상위 3,458개 | 4.708~4.760 | 0.433~0.464 | **0.06** | 0.74 |

세 전략 모두 ρ≈0, p>0.7 → **통계적으로 완전히 무의미**.

**왜 ρ≈0인가**: 54,912개에서 2,000개를 어떻게 뽑아도 Vendi와 ODD entropy가 거의 같은 값이 나올 만큼 데이터가 균질하게 집중되어 있습니다. Vendi 변동폭 0.06, ODD H 변동폭 0.06 — 두 값 모두 run 간 변동이 너무 작아 상관을 측정할 신호 자체가 없습니다.

**두 검증의 종합 해석**:

```
쌍별 Spearman ρ=0.24 (partial)
  → 임베딩 벡터가 ODD 조건 차이를 약하게 인코딩함 (벡터 특성)
  → Vendi는 ODD 다양성의 부분적 대리 지표로만 사용 가능

Per-run Spearman ρ≈0 (n=30)
  → Vendi score 수치와 ODD entropy 수치가 서로 독립적으로 움직임
  → 두 지표는 같은 클립으로 계산해도 서로 다른 차원을 측정
...
```
Effective N = 3,458  (6.3%)   — 로컬하게 독특한 샘플
      ↓ 그 3,458개를 다시 Vendi로 보면
Vendi       = 3.5             — 그 3,458개조차 3~4개 방향에만 집중
```
...
| 지표 | 값 | 의미 |
|------|-----|------|
| median LID | **12.95** | 클립 주변 평균 변동 축 수 — GMM 임계값(16.2)보다 낮음 |
| mean LID | 13.60 | 중앙값보다 높음 → 고LID 클립이 평균을 끌어올림 |
| P10 | 8.68 | 하위 10% — 매우 단조로운 클립 |
| P90 | 19.27 | 상위 10% — 다양한 변주 공간 존재 |
| lid_reliable_ratio | **100.0%** | k=20 이웃 거리 r_max < 0.6 → 전 클립 LID 신뢰 가능 |

**분포 해석**: 중앙값(12.95)이 GMM 임계값(16.2)보다 낮으므로 **과반수 클립이 저LID(Q1/Q3 후보)**입니다. P10=8.68은 일부 클립이 사실상 1~2차원 구조(직선 반복)임을 시사합니다. lid_reliable_ratio=100%는 이번 데이터셋이 초고밀도(이웃이 항상 가까움)이기 때문에 r_max가 작아 LID 추정 자체는 안정적입니다.
...
|------|-----------------|-----------|------|
| 0-C (중앙값 12.95 기준) | **0.052** | > 0.05 → flipd_recommended=True | 원본 추정 |
| 0-D (GMM 임계값 16.2 재계산) | **0.035** | < 0.05 → flipd_recommended=False | 확정값 |

```
k_sensitive_rate = (k=20 저LID 판정이지만 k=15 또는 k=25에서 고LID) / 전체 저밀도 신뢰 클립
```

- **0-C 단계**: 임시 임계값(중앙값 12.95)으로 계산 시 0.052 > 0.05 → FLIPD 실행 권고
- **0-D 단계**: GMM 확정 임계값(16.2)으로 재계산 시 0.035 < 0.05 → FLIPD 불필요 판정
- **결론**: GMM 임계값 16.2가 클립 분포의 자연 경계이므로 경계 클립 수가 줄어들어 k-민감도가 낮아진 것. 실제 FLIPD는 `q3_boundary_rate=38.4% > 0.3` 조건으로 트리거됨
...
### 3-5. 6-분류 Action Map 결과 (0-D / 0-D-val)

### FLIPD 전/후 비교

| 분류 | 레이블 | FLIPD 전 | FLIPD 후 | 비고 |
|------|--------|-----------|----------|------|
| Q0 | KEEP | 6,499 (11.8%) | 변동 없음 | 잘 수집된 다양한 클립 |
| Q1 | PRUNE | 32,988 (60.1%) | 변동 없음 | 밀집+저차원 = 중복 제거 대상 |
| Q2 | COLLECT | 6,408 (11.7%) | **9,868 (18.0%)** | +3,460 (FLIPD 업그레이드) |
| Q3 | EVALUATE | 9,017 (16.4%) | **5,557 (10.1%)** | -3,460 → Q2로 이동 |
| Q4 | LID_UNCERTAIN | 0 (0.0%) | 변동 없음 | 고립 클립 없음 |
| Q5 | PRUNE_UNCERTAIN | 0 (0.0%) | 변동 없음 | |

- GMM 임계값: `density=0.9335 (K=3)`, `lid=16.1994 (K=3)`
- 두 분포 모두 3성분 혼합으로 분리됨 (단봉 없음)

### ⚠️ FLIPD 결과 해석 주의

```
upgrade_rate       = 1.000   (3,460/3,460 전원 Q3→Q2)
median_correction  = 184.86  (정상 범위 초과)
```

**원인**: 데이터가 초고밀도(kNN 거리 0.05~0.07로 균일)하여 `r_j/r_k ≈ 1.0` →
FLIPD 경계 보정항 `j*(r_j/r_k)^mle`이 수치 발산 → 200으로 클리핑 → 전원 임계값 초과.

**의미**: 이 3,460개 Q3→Q2 업그레이드는 FLIPD 공식이 초고밀도 데이터에서 발산한 결과이므로
...
**K=12 선택** (실루엣 flat_fallback=True, 범위=0.003 — TF-IDF 공간 클러스터 구조 약함)

| S | 크기 | Q0% | Q1% | Q2% | Q3% | Vendi | 주요 키워드 | 비고 |
|---|------|-----|-----|-----|-----|-------|------------|------|
| S0 | 5,940 | 8.2 | 70.2 | 13.0 | 8.6 | 2.8 | vehicle, road, lane | Q1 과다 |
| S1 | 2,998 | 8.2 | 32.8 | 39.5 | 19.5 | 3.0 | vehicle, pedestrian | Q2 많음 |
| S2 | 5,141 | 13.8 | 71.0 | 10.1 | 5.1 | 2.5 | lane, sedan, suv | Q1 과다 |
| **S3** | 3,048 | 9.6 | **83.3** | 4.7 | 2.4 | 2.3 | vehicle, headlights | **Q1 극단 (야간 단조)** |
| S4 | 4,829 | 10.0 | 38.2 | 33.9 | 17.9 | 2.9 | turn, vehicle, road | Q2 많음 |
| S5 | 5,599 | 12.8 | 71.5 | 10.1 | 5.5 | 2.5 | lane, vehicle, highway | Q1 과다 |
| S6 | 5,306 | 11.4 | 62.8 | 15.3 | 10.5 | 2.5 | light, intersection | |
| S7 | 6,271 | 17.6 | 60.2 | 16.7 | 5.5 | 2.6 | parked, vehicle | COLLECT 후보 |
| S8 | 2,767 | 5.4 | 63.8 | 15.6 | 15.1 | 2.6 | vehicle, travels | |
| S9 | 4,362 | 9.2 | 43.6 | 29.4 | 17.8 | 2.9 | vehicle, curve | Q2 많음 |
| S10 | 1,699 | 4.9 | 45.0 | 26.7 | 23.4 | 2.6 | parking lot | Q3 많음 |
| S11 | 6,952 | 17.5 | 61.3 | 14.6 | 6.6 | 2.6 | vehicle, lane | COLLECT 후보 |

**두 공간 독립성**: NMI=0.0346, ARI=0.0146 → `two_space_independence_ok=True` ✓
(TF-IDF 시나리오 분류 ↔ 임베딩 사분면 분류가 독립적 — 교차표 유효)

**주목 포인트**:
- **S3 (야간/헤드라이트)**: Q1=83.3% — 밤 직진 주행만 과도 수집된 가장 단조로운 시나리오
- **S1, S4, S9**: Q2 비율 30~40% — 수집되지 않은 다양한 케이스 상대적으로 많음
- 모든 시나리오 Vendi 2.3~3.0: **시나리오 내부도 다양성 낮음**
- **억압 계수(vendi_dedup/vendi_random)**: 전 시나리오 1.020~1.057 (평균 1.031) — VENDI_SUPPRESSION_HIGH=2.0 미달 → 중복 제거만으로는 다양성 개선 기대 어려움. **S6(교차로)=1.057** 가 가장 높아 중복 제거 효과가 다소 큼
- `caution_scenarios=[]`: PRUNE_THRESH=90.15% (1.5 × 60.1%)로 설정 → 어떤 시나리오도 위험 판정 미달
- `healthy_scenarios=[]`: 어떤 시나리오도 Q0 ≥ 40% + size ≥ 500 기준 미달

---

## 5. 갭 슬라이스 분석 — 수집/합성 우선순위 (0-E-2)

### COLLECT 후보 (7개)

수집 우선순위 결정 기준: ① LID 기반 (mean_lid ≥ 16.2 또는 gap_in_scenario_ratio ≥ 0.4), ② ODD 기반 (odd_effective_n ≥ 22.0). 두 기준 중 하나라도 충족 시 COLLECT_HIGH_PRIORITY.

#### HIGH PRIORITY (5개) — LID 또는 ODD 기준 충족

| 시나리오 | 갭 수 | Q2 유효 N | mean LID | ODD eff_n | 승격 근거 | 부족한 유형 |
|---------|-------|----------|----------|----------|----------|------------|
| **S4** (회전/교차로) | 2,500 | 133.4 | 16.16 | 22.04 | **ODD** | roundabout, cyclist, rural, overcast |
| **S1** (보행자/감속) | 1,768 | 99.2 | 16.11 | 23.65 | **ODD** | yielding, bus, tram, crossing |
| **S11** (일반 주행) | 1,475 | 82.3 | 16.74 | 28.70 | LID+ODD | roundabout, 습노면, construction zone, bus |
| **S5** (램프/터널) | 876 | 45.4 | 16.11 | 22.30 | **ODD** | ramp, tunnel, 감속 |
| **S8** (버스/트럭) | 852 | 35.9 | 14.41 | 22.42 | **ODD** | bus, truck, wet, highway |

- S4·S1·S5·S8: mean_lid < 16.2 — 임베딩 공간 기준으로는 단조하지만 **ODD 다양성(≥22)으로 승격**. 임베딩(ρ=0.21 partial)이 ODD 조건 변화를 완전히 포착하지 못해 LID만으로 판단하면 놓치는 케이스.
- S11: LID(16.74) + ODD(28.7) 둘 다 충족. `lid_context_caution=True` (시나리오 내 희귀 케이스).

#### NORMAL (2개) — LID 기반

| 시나리오 | 갭 수 | Q2 유효 N | mean LID | ODD eff_n | 부족한 유형 |
|---------|-------|----------|----------|----------|------------|
| **S7** (주차 주변) | 1,392 | 83.3 | 17.17 | 15.97 | cyclist, bus, truck, 교차로 회전 |
| **S2** (sedan/SUV) | 780 | 42.2 | 16.38 | 21.57 | ramp, 감속, urban stop |

> 억압 계수 전 후보 1.02~1.04 — VENDI_SUPPRESSION_HIGH=2.0 미달. 모든 후보가 중복 때문이 아닌 **해당 유형 클립 자체가 수집되지 않은** 구조적 갭임.

### SYNTHETIC 후보 (5개)

> S0·S6·S9: 이전 실행 대비 S1·S4·S5·S8 이탈 (ODD 기반 COLLECT_HIGH_PRIORITY 승격)
> 남은 5개는 odd_effective_n ≤ 22.0 + mean_lid < 16.2 → 임베딩·ODD 두 기준 모두 미달

> 모두 `partial_collect_flag=True` → 합성 전 기존 Q2 클립 별도 수집 먼저 권장

| 시나리오 | 갭 클립 수 | mean LID | ODD eff_n | 주요 키워드 |
|---------|-----------|----------|----------|------------|
| S9 | 2,058 | 15.63 | 15.42 | curve, road |
| S0 | 1,284 | 15.52 | 16.25 | road, lane |
| S6 | 1,370 | 15.38 | 21.76 | light, intersection |
| S10 | 852 | 14.78 | 15.07 | parking lot |
| S3 | 218 | 15.71 | 8.31 | headlights |
...
### 즉시 검토
- [ ] **FLIPD 이슈 확인**: Q3 클립 수동 샘플링 → 실제로 Q2(다양)인지 Q3(단조)인지 육안 확인
- [ ] **lid_threshold 재검토**: 현재 16.2 → 13~14 범위 하향 조정 후 재실행 고려

### EXP-004 데이터 수집

**HIGH PRIORITY (ODD 기반 신규 포함)**
- [ ] **S4 갭** (roundabout/cyclist/rural): 2,500클립, Q2 유효 N=133 — 가장 큰 갭
- [ ] **S1 갭** (보행자/버스/tram/양보): 1,768클립, Q2 유효 N=99
- [ ] **S11 갭** (roundabout/습노면/공사): 1,475클립, Q2 유효 N=82
- [ ] **S5 갭** (ramp/터널/감속): 876클립, Q2 유효 N=45
- [ ] **S8 갭** (버스/트럭/wet highway): 852클립, Q2 유효 N=36

**NORMAL (LID 기반 기존)**
- [ ] **S7 갭**: cyclist, bus, truck 포함 주차 주변 1,392클립
- [ ] **S2 갭**: ramp 진입/이탈, urban 감속 정지 780클립

### 데이터 합성
- [ ] 5개 SYNTHETIC 후보 (S9/S0/S6/S10/S3) — 시나리오별 Q2 클립 먼저 분리 수집 후 합성 생성

### Pruning (EXP-004)
- [ ] **S3 (야간 직진)**: Q1=83.3% — 첫 번째 pruning 대상
- [ ] **Q1 전체 32,988개** 중 유효 다양성 보존하면서 제거 전략 수립
...
| `flipd_upgraded_clips.npy` | npy (M,) | Q3→Q2로 업그레이드된 클립 인덱스 배열. M=3,460개 (신뢰도 주의) | ★ |
...
odd_diversity
  .found_ratio          : ODD 파일 존재 클립 비율 → 1.0 (54,912/54,912, 전 클립 커버)
  .n_unique_combos      : 발견된 고유 ODD 조합 수 → 422 (최대 25,920 중 1.6%)
  .mean_norm_entropy    : 7개 차원 정규화 엔트로피 평균 → 0.413
  .per_dim              : 차원별 엔트로피·고유값 수 (이번 실행 결과)
    .road_type          : n_unique=6, norm_entropy=0.591  (중간)
    .weather            : n_unique=4, norm_entropy=0.206  (맑음 편중)
    .traffic_density    : n_unique=4, norm_entropy=0.487  (중간)
    .agent_type         : n_unique=6, norm_entropy=0.083  ⚠️ cars_only 극도 집중
    .lighting           : n_unique=3, norm_entropy=0.909  (비교적 고름)
    .scene_ambiguity    : n_unique=3, norm_entropy=0.035  ⚠️ 단일 카테고리 집중
    .road_divider       : n_unique=5, norm_entropy=0.581  (중간)
...
odd_topk_diversity          : 고유성 상위 클립(uniqueness_weight 기준 상위 Effective_N개)의 ODD 분포
  .n_clips              : 3,458 (= Effective N, 전체의 6.3%)
  .n_unique_combos      : 219 (전체 422 중 52% 커버)
  .odd_effective_n      : 41.187 (전체 37.375보다 높음 → 고유 클립의 ODD 분포가 더 고름)
  .mean_norm_entropy    : 0.437 (전체 0.413보다 높음 → 고유 클립이 ODD 관점에서도 더 다양)
  .per_dim              : 차원별 상세 (전체 odd_diversity와 비교)
    .road_type          : norm_entropy=0.654  (전체 0.591 ↑)
    .weather            : norm_entropy=0.299  (전체 0.206 ↑)
    .traffic_density    : norm_entropy=0.504  (전체 0.487 ↑)
    .agent_type         : norm_entropy=0.168  (전체 0.083 ↑)
    .lighting           : norm_entropy=0.821  (전체 0.909 ↓ — 임베딩 고유성 무관)
    .scene_ambiguity    : norm_entropy=0.067  (전체 0.035 ↑)
    .road_divider       : norm_entropy=0.547  (전체 0.581 ↓ — 임베딩 고유성 무관)
```

#### `thresholds.json`
```
density_threshold  : 고밀도/저밀도 경계 (GMM BIC K=3 교차점) = 0.9335
lid_threshold      : 고LID/저LID 경계 (GMM BIC K=3 교차점)   = 16.1994
